...
class NodesController:  # pylint: disable=too-few-public-methods
    ...
    def add_node(self):
        ...
        if errors:
            ...
            errmsg = ", ".join(
                [f"Key: [{key}], Error: {error}" for key, error in errors.items()]
            )
    ...
    @cherrypy.tools.json_in()
    @cherrypy.tools.json_out()
    def delete_node(self, name):  # pylint: disable=unused-argument
        """
